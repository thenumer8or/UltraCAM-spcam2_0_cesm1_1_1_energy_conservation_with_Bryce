...
!=====================================================================
  ...
  use abortutils  ,only:endrun     !CRT 2018/03/27: changed from cam_abortutils
...
#ifdef LIBERATE_BL
   real(r8)      :: dlat,dlon,my_lat,my_lon,my_lon_shifted,fhorizontal,fvertical,fvertical_reverse,fnep,fsep,fnamb,ftots,ftots_reverse,ftotu,ftotv
#endif
...
! Below HP is porting MSP's implementation of T-nudging hole. See email dated July 25, 2018.
! HP: Not sure if phys_state%pmid handle is accessible in this routine.
...
! Horizontal limiting of the nudging: see MSP's email dated Oct. 21, 2018.
      my_lat = get_rlat_p(lchnk, i)*57.296_r8
      my_lon = get_rlon_p(lchnk, i)*57.296_r8
...
! Coefficient to horizontally limit the nudging
      fhorizontal = fnep + fsep + fnamb

       do k=1,pver
! Coefficient to vertically limit the nudging
         fvertical         = (1. - 1./(1. + exp(-(phys_state%pmid(i,k)/100.-625.)/9.)))    !zero below "that pressure level after 100."
         fvertical_reverse = 1.0 - fvertical                                               !one below, and zero above that pressure level.  

! Combine the horizontal and vertical nudging coefficients
         ftots_reverse = fvertical_reverse * fhorizontal
         ftots         = 1.0 - ftots_reverse 
         ftotu = 1.0 
         ftotv = 1.0 

! Limit the nudging
         phys_tend%s(i,k) = phys_tend%s(i,k) * ftots 
         phys_tend%u(i,k) = phys_tend%u(i,k) * ftotu
         phys_tend%v(i,k) = phys_tend%v(i,k) * ftotv
       end do
     end do
#endif

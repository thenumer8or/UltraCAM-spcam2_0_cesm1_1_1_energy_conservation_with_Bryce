...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    call aerosol_emis_intr (state, ptend, cam_in, ztodt)
...
subroutine tphysbc (ztodt,               &
    ...
    use carma_flags_mod, only: carma_do_detrain, carma_do_cldice, carma_do_cldliq
...
!HP    write(6, *) 'HP flag 0: before call to crm_physics_tend. Inside subroutine: tphysbc'
    call t_startf('whole_crm_physics_tend')
    if (use_SPCAM) then
      call crm_physics_tend(ztodt, state, tend, ptend, pbuf, dlf, cam_in, cam_out)

   endif
    call t_stopf('whole_crm_physics_tend')
